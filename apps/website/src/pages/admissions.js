...
const InfoCard = ({ icon, title, value, detail }) => (
  <Card>
    ...
    <atoms.Typography as="h3" variant="primary" size="xl2" textWeight="bold" className="text-white mb-2">
      {title}
    </atoms.Typography>
    {value && (
      <atoms.Typography as="p" variant="primary" size="body_lg" textWeight="bold" className="text-blue-400 mb-2">
        {value}
      </atoms.Typography>
    )}
...
export default function Admissions() {
  ...
  const atAGlanceItems = [
    { icon: '💰', title: 'Tuition', value: '$0', detail: 'Dev Launchers is a 501(c)(3) nonprofit' },
    { icon: '📝', title: 'Admissions', value: 'Rolling', detail: 'We review weekly and place as capacity opens' },
    { icon: '⏰', title: 'Time', value: '5–10 hrs/week', detail: '2 live + 3–8 async hours' },
    { icon: '🌍', title: 'Sessions', value: 'Central Time', detail: 'Global participation welcome' },
    { icon: '👥', title: 'Cohorts', value: 'Quarterly', detail: 'Q1–Q4 reporting periods' },
  ];

  const startingOptions = [
    { title: 'Apply to an open opportunity', description: "If there's a role that fits, apply on our Open Opportunities page.", icon: '🎯', cta:"View Opportunities", link:"https://devlaunchers.org/join" },
    { title: 'Join the Talent Network', description: 'If nothing fits (or teams are full), join our Talent Network to be considered for future roles that match your skills and goals.', icon: '🌟' , cta:"Join" , link:"https://devlaunchers.org/join/second" },
  ];

  const admissionsProcess = [
    { label: 'Rolling admissions', detail: 'We accept applications year-round, based on mentor availability and team capacity.' },
    { label: 'Review cadence', detail: 'Applications are reviewed weekly.' },
    { label: 'Placement timing', detail: 'Most placements align to the start of the next 4-week sprint.' },
    { label: 'Placement factors', detail: 'Skills, goals, time commitment, and open roles on product teams. If teams are full, you can join our talent community to be considered for new openings.' },
  ];
  ...
  const requirements = [
    ...
    { title: 'Synchronous sessions', detail: 'Attend weekly mentor-led collaboration sessions (Central Time; recordings/notes shared when possible)', icon: '🎥' },
    { title: 'Asynchronous communication', detail: 'Maintain regular updates and check-ins via our team channels (Discord, GitHub)', icon: '💬' },
    { title: 'Collaboration standards', detail: 'Contribute respectfully in a diverse, global community and align to shared goals', icon: '🤝' },
  ];

  const cohortCriteria = [
    { title: 'Synchronous participation', detail: 'Attended ≥2 live collaboration sessions during the quarter', icon: '🎬' },
    { title: 'Async participation', detail: 'Posted ≥2 meaningful async updates in official channels (Discord/GitHub)', icon: '💻' },
    { title: 'Sprint progress', detail: 'Demonstrated progress toward completing at least one assigned issue', icon: '📊' },
  ];

  const includedItems = [
    { icon: '🧑‍🏫', title: 'Mentor-led sessions', detail: 'Design Collaboration and Cross-Department Collaboration each week.' },
    { icon: '✅', title: 'Project assignments', detail: 'GitHub issues with clear acceptance criteria and DoD.' },
    { icon: '🧰', title: 'Design system access', detail: 'Use our component libraries and tokens in real work.' },
    { icon: '🔍', title: 'Reviews & retros', detail: 'Code/design reviews, research readouts, and sprint retrospectives.' },
    { icon: '🗂️', title: 'Portfolio artifacts', detail: 'PRs, prototypes, reports, and component stories you can show.' },
  ];

  const applicationSteps = [
    { title: 'Review', description: 'Review the Curriculum and this page' },
    { title: 'Choose', description: 'Apply to Open Roles or Join the Talent Network' },
    { title: 'Placement', description: "We'll place you based on your goals, skills, and team capacity" },
    { title: 'Start', description: 'Start with a well-scoped issue, meet your mentor, and join the sprint cadence' },
  ];
    ...
    <>
      ...
      <main className="flex flex-col items-center min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white">
          ...
          <div className="text-center gap-6">
            ...
            <HighlightBox variant="warning">
              <atoms.Typography as="p" variant="secondary" size="body_md">
                <span className="font-bold">Note:</span> We cannot sponsor visas for international members; international participants are welcome to join remotely.
              </atoms.Typography>
            </HighlightBox>
          </div>
        </Section>
        ...
        <Section variant="card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            {startingOptions.map((option) => (
              <Card key={option.title}>
                ...
                <atoms.Button     mode="dark" type="secondary"iconPosition="undefined" as="a" status="default"href={option.link} >
                  {option.cta} 
                </atoms.Button>
              </Card>
            ))}
          ...
          <HighlightBox variant="warning">
            <atoms.Typography as="p" variant="secondary" size="body_md">
              <span className="font-bold">Capacity note:</span> Mentor bandwidth limits how many members we can onboard at a time. Placement isn't guaranteed, even for strong applicants.
            </atoms.Typography>
          </HighlightBox>
        </Section>
