...
```bash
$ yarn add --dev eslint
$ yarn add --dev @devlaunchers/eslint-config-bases:"workspace:^"
```
...
> **Tip:** "@devlaunchers/eslint-config-bases/prettier" must be set at the end to disable any
> conflicting rules.
...
To prevent conflicts between prettier and eslint, you must re-export the prettier base from `@devlaunchers/eslint-config-bases`.

```javascript
const { getPrettierConfig } = require("@devlaunchers/eslint-config-bases/helpers");
module.exports = {
  ...prettierConfig,
  overrides: [
    // whatever you need
